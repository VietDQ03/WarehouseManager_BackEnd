...
const edit = async (id, { in_price, quantity_real, quantity_doc }) => {
  try {
    // Get old inOrder to compare quantities
    const oldInOrder = await inOrders.findById(id);
    if (!oldInOrder) {
      throw new Error('InOrder not found');
    }

    // Calculate quantity difference
    const quantityDiff = quantity_real - oldInOrder.quantity_real;

    // Update inOrder
    const updatedInOrder = await inOrders.findByIdAndUpdate(
      id,
      {
        in_price,
        quantity_real,
